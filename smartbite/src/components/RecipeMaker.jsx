...
import { inventoryApi, recipesApi } from '../api/smartbiteApi';
const RecipeMaker = () => {
  ...
  const ingredientsracipe = async () => {
    ...
    try {
      ...
      const presentdata = res.data.items;
  ...
  const fetchRecipe = async () => {
    ...
    try {
      ...
      const response = await recipesApi.getNutritionRecipes({ maxCalories, maxProtein, maxCarbs, maxFat });
   ...
   setLoading(false)
    return 
      ...
      const response = await recipesApi.getRecipeMaker({ ingredients, diet });
      setRecipes(response.data);
    }
    } catch (err) {
