...
const MealPlanner = () => {
  ...
  const handleDeleteSave = async (timeframe) => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast("You must log in to delete meal plans.");
      return;
    }
    ...
    try {
      ...
      console.error("Error:", error);
      toast("An unexpected error occurred while deleting the meal plan.");
    }
  };
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white font-poppins">
      ...
      <div className="mt-28 mb-20">
        <div className="mx-auto mb-12 px-6">
          ...
          <div className="w-full p-8   flex justify-center"><span className="flex gap-24 "> <button onClick={() => { setshowsaved(true); handlefetchSave("day"); settime("day") }} className="px-16 py-3  m-auto  bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-lg hover:from-indigo-500 hover:to-purple-600 transition transform hover:scale-110">
              Show Saved Day plan
            </button>  <button onClick={() => { setshowsaved(true); handlefetchSave("week"); settime("week") }} className="px-16 py-3  m-auto  bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-lg hover:from-indigo-500 hover:to-purple-600 transition transform hover:scale-110">
            Show Saved Week plan
          </button>
          ...
          {mealPlan && (
            <div ref={detailsRef} className="mt-12">
              ...
              <div className="w-full  flex justify-center">
                {showsaved ? <button onClick={() => handleDeleteSave(time)} className="px-16 py-4 m-auto  bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-lg hover:from-indigo-500 hover:to-purple-600 transition transform hover:scale-110">
                  Remove Diet
                </button> : <button onClick={() => handleSaveDiet()} className="px-16 py-4 m-auto  bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-lg hover:from-indigo-500 hover:to-purple-600 transition transform hover:scale-110">
                  {!saved ? "Save Diet" : "Saved"}
                </button>}
              </div>
            </div>
          )}
