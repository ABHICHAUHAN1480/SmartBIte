...
const scrollToFeatures = () => {
  if (detailsRef.current) {
    detailsRef.current.scrollIntoView({
      behavior: "smooth",
      block: "nearest",  // Align the top of the element with the top of the viewport
    });
  }
};


// Trigger scroll when selectedRecipe is set
useEffect(() => {
  if (selectedRecipe) {
    scrollToFeatures();
  }
...
const searchRecipes = async () => {
    ...
    setLoading(true)
    try {
      const response = await axios.get("https://smartbite-g813.onrender.com/api/search", {
        params: { query },
      });
    ...
    <span
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col items-center bg-gray-800 p-6 rounded-xl shadow-2xl max-w-lg mx-auto transition-transform duration-300 hover:scale-105">
          <h1 className="text-xl sm:text-4xl mb-6 font-bold font-serif tracking-wide text-blue-400">
            Recipe Search
          </h1>
          <input
            type="text"
...
>
      ...
      {[
        ...
        >
          <div className="relative overflow-hidden rounded-md">
            ...
            />
             <span  onClick={() => handleFav(recipe.ak)}   className="absolute z-20 top-0 right-0">
  <lord-icon
  ...
  <div
    ref={detailsRef}
    className="recipe-details flex flex-col lg:flex-row gap-6 lg:gap-12 bg-gradient-to-br from-gray-900 via-black to-gray-800 p-6 sm:p-8 lg:p-12 rounded-3xl shadow-2xl mt-8 transform transition-all duration-300"
  >
  
    <div className="flex-1 flex flex-col items-center lg:items-start">
      <h2 className="text-2xl sm:text-3xl lg:text-4xl font-serif font-bold text-yellow-400 mb-4 lg:mb-6 text-center lg:text-left hover:scale-105 transition-transform duration-300">
      ...
      <div className="relative group">
        <img
          ...
          className="w-full sm:w-[90%] lg:w-[750px] h-auto object-cover rounded-3xl shadow-lg transition-transform duration-300 hover:scale-110 hover:shadow-2xl"
        />
    ...
    <div className="flex-1 overflow-y-auto h-[50vh] sm:h-[60vh] lg:h-[70vh] bg-gray-800 bg-opacity-90 border border-gray-700 p-6 sm:p-8 rounded-3xl shadow-inner">
   
      <div className="mb-6 lg:mb-8">
        <h3 className="text-2xl sm:text-3xl font-serif font-extrabold text-yellow-400 mb-4 lg:mb-6 border-b border-yellow-500 pb-2">
          Ingredients
        </h3>
        <ul className="list-disc list-inside space-y-3 sm:space-y-4 text-gray-300 text-sm sm:text-lg leading-relaxed">
          {selectedRecipe.extendedIngredients.map((i, index) => (
            <li key={index} className="hover:text-yellow-400 transition-colors duration-200">
              {i.original}{" "}
              <span className="text-gray-400 text-sm hidden sm:flex">({i.originalName})</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="mb-6 lg:mb-8">
        ...
        <p className="text-gray-300 text-sm sm:text-lg leading-loose">
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full py-3 px-4 sm:px-6 rounded-full bg-gradient-to-r from-green-500 via-green-600 to-green-700 text-white font-semibold text-sm sm:text-lg shadow-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl hover:from-green-600 hover:via-green-700 hover:to-green-800"
      >
      ...
      <a
        ...
        className="flex items-center justify-center gap-2 w-full py-3 px-4 sm:px-6 mt-4 rounded-full bg-gradient-to-r from-yellow-500 via-yellow-600 to-yellow-700 text-gray-900 font-semibold text-sm sm:text-lg shadow-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl hover:from-yellow-600 hover:via-yellow-700 hover:to-yellow-800"
      >
